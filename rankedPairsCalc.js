...
const tally = (pairs, higher, lower, count) => {
  ...
  if (!newPairs[key]) {
    ...
    newPairs[key] = {
      ...newPairs[key],
      [higher]: newPairs[key][higher] + count,
    };
  }
  return newPairs;
};

export const makePairs = (pollResult, options) => pollResult.reduce((pairs, datum) => {
  const { vote, lowVote, count } = datum;
  ...
  vote.forEach((higher, index) => { // high votes beat unranked and low votes
    if (options.includes(higher)) { // data validation
      vote.slice(index + 1).forEach((lower) => {
        if (options.includes(lower)) {
          newPairs = tally(newPairs, higher, lower, count);
        }
      });
  ...
  lowVote.forEach((higher, index) => {
    if (options.includes(higher)) {
      lowVote.slice(index + 1).forEach((lower) => {
        if (options.includes(lower)) {
          newPairs = tally(newPairs, higher, lower, count);
        }
      });
...
export const rankPairs = (pairs) => Object.values(pairs).sort((pair1, pair2) => {
...
export const makeNodes = (rankedPairs, options) => {
  let nodes = options.reduce((object, option) => ({
    ...object,
    [option]: {
      above: [],
      below: [],
...
export const makeRanks = (nodes, pairs, options) => {
  const rankings = [[]];
  const ratios = [];
...
export function fromPairsCalc(pairs = {}, options = []) {
  const rankedPairs = rankPairs(pairs);
  const nodes = makeNodes(rankedPairs, options);
